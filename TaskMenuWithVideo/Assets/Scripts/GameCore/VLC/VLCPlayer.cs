...
public class VLCPlayer : MonoBehaviour
{
	...
	private string _path; //Can be a web path or a local path
	...
	void Awake()
	{
		...
		_path = _settingsController.StreamUrl;
	...
	public void Open()
	{
		...
		var trimmedPath = _path.Trim(new char[]{'"'});//Windows likes to copy paths with quotes but Uri does not like to open them
		mediaPlayer.Media = new Media(new Uri(trimmedPath));
		Play();
		if (Tracks(TrackType.Audio).Count != 0 && _settingsController.IsMusicEnabled)
		{
